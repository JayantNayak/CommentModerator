...
public class ModeratorAPI {
	...
	@ApiOperation(value = "Evaluate Comment")
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Successfully posted"),
			@ApiResponse(code = 401, message = "You are not authorized to view the resource"),
			@ApiResponse(code = 403, message = "Accessing the resource you were trying to reach is forbidden"),
			@ApiResponse(code = 500, message = "Invalid params"),
			@ApiResponse(code = 404, message = "The resource you were trying to reach is not found") })
	@PostMapping("moderate")
	public ResponseComment evaluateComment(@RequestBody RequestComment req){
		
		return moderatorService.analyzeComment(req);
	}
